...
/**
 * Port the real Pragmatic Play games into the rebuild as pure DB config — the
 * same pipeline as {@see ImportAmaticGamesCommand} / {@see ImportEgtGamesCommand}
 * ({@see EgtGameParser} is reused, generalised to also read Pragmatic's
 * bare-numeric `Paytable[N]` / `$wild = 'N'` convention). Bet-per-line ladder
 * is read at runtime from the legacy `games.bet` column, same as Amatic.
 *
 * Unlike Amatic/EGT/Playtech, the bundle ships a real root `index.html` (no
 * synthesised shell needed) — {@see BundleEntryResolver} finds it generically.
 * `client_protocol` is `pragmatic`.
 *
 * Desktop titles only (`*PM`, e.g. `AncientEgyptPM` — NOT `*PT`, which is
 * legacy Playtech, a different, unrelated protocol). The `*PMM` mobile
 * variants are separate legacy game codes, not yet ported.
 *
 *   php artisan pragmatic:import
 *   php artisan pragmatic:import --only=AncientEgyptPM,AztecGemsPM
...
class ImportPragmaticGamesCommand extends Command
{
    ...
    protected $description = 'Import the real Pragmatic Play games as DB-driven templates + per-shop games';
    ...
    public function handle(BundleManager $bundles): int
    {
        ...
        $pragmatic = Category::firstOrCreate(
            ['slug' => 'pragmatic'],
            ['title' => 'Pragmatic Play', 'position' => 6, 'config' => ['client_protocol' => 'pragmatic']],
        );
        ...
        foreach ($codes as $code) {
            ...
            try {
                ...
                $title = Str::title(preg_replace('/(?<!^)(?=[A-Z])/', ' ', Str::before($code, 'PM')));
                ...
                if (! $this->option('skip-bundles') && ($this->option('fresh-bundles') || ! $template->activeBundle)) {
                    ...
                    if (is_dir($src)) {
                        $bundle = $bundles->storeFromDirectory(
                            $template, $src,
                            notes: 'Pragmatic Play HTML5 front-end (pragmatic:import).',
                        );
